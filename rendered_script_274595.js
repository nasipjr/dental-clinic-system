    ...
    // ── Translation Script for Arabic Language ──
    (function() {
        const arabicTranslations = {
            ...
            "Configure automated reminders via Email, SMS, and WhatsApp": "تهيئة التذكيرات التلقائية عبر البريد الإلكتروني ورسائل SMS وتيليغرام",
            ...
            "Enable Telegram Reminders (Free — Recommended for Syria)": "تفعيل التذكير عبر تيليغرام (مجاني — موصى به لسوريا)",
            "Sends appointment reminders via Telegram Bot. Completely free — no per-message cost.": "يرسل تذكيرات المواعيد عبر بوت تيليغرام. مجاني تماماً بدون تكلفة لكل رسالة.",
            "Enable SMS Reminders (CommPeak — Syriatel & MTN Syria)": "تفعيل التذكير عبر رسائل SMS (CommPeak - يدعم سيريتيل وMTN سوريا)",
            "Sends text messages to patient mobile numbers via CommPeak gateway (supports Syrian numbers).": "يرسل رسائل نصية لأرقام المرضى عبر بوابة CommPeak (يدعم الأرقام السورية).",
            "Enable Email Reminders (Gmail SMTP)": "تفعيل التذكير عبر البريد الإلكتروني (Gmail)",
            "Sends reminder emails to patient email addresses using Gmail or any SMTP server.": "يرسل رسائل تذكير للمرضى عبر Gmail أو أي خادم SMTP آخر.",
            "Telegram Bot Settings": "إعدادات بوت تيليغرام",
            "Create a bot via @BotFather on Telegram and paste the token below.": "أنشئ بوتاً عبر @BotFather على تيليغرام والصق الرمز أدناه.",
            "The patient must send /start to the bot first to get a Chat ID.": "يجب على المريض إرسال /start للبوت أولاً للحصول على Chat ID.",
            "Telegram Bot Token": "رمز بوت تيليغرام",
            ...
            "Automated Reminder Settings": "إعدادات التذكير التلقائية",
            ...
            "Email Subject (24h)": "عنوان البريد الإلكتروني (قبل 24 ساعة)",
            "Enter subject for 24h email": "أدخل عنوان البريد للتذكير قبل 24 ساعة",
            "Enter email template body for 24h reminder": "أدخل نص رسالة البريد للتذكير قبل 24 ساعة",
            "Email Subject (2h)": "عنوان البريد الإلكتروني (قبل ساعتين)",
            "Enter subject for 2h email": "أدخل عنوان البريد للتذكير قبل ساعتين",
            "Enter email template body for 2h reminder": "أدخل نص رسالة البريد للتذكير قبل ساعتين",
            "Enable 24-Hour Reminder": "تفعيل تذكير قبل 24 ساعة",
            "Enter message template for 24h reminder": "أدخل قالب الرسالة للتذكير قبل 24 ساعة",
            "Enable 2-Hour Reminder": "تفعيل تذكير قبل ساعتين",
            "Enter message template for 2h reminder": "أدخل قالب الرسالة للتذكير قبل ساعتين",
            "Available dynamic tags:": "الرموز الديناميكية المتاحة:",
            "Test Chat ID (Your Telegram Chat ID)": "معرف الدردشة للتجربة (Telegram Chat ID الخاص بك)",
            "Test Telegram": "تجربة تيليغرام",
            "Test Phone Number (e.g., +963958948727)": "رقم الهاتف للتجربة (مثال: 963958948727)",
            "Send Test SMS": "إرسال SMS تجريبية",
            "Test Email Address": "عنوان البريد الإلكتروني للتجربة",
            "Send Test Email": "إرسال بريد إلكتروني تجريبي",
            "Upload Medical File / X-Ray": "رفع ملف طبي / صورة أشعة",
            "Select an X-ray, panorama image, document, or PDF report to attach to this patient's clinical file.": "اختر صورة أشعة، صورة بانوراما، مستند، أو تقرير PDF لربطه بالملف السريري للمريض.",
            "Select File": "اختر الملف",
            "Description / Notes": "الوصف / الملاحظات",
            "Upload File": "رفع الملف",
            "e.g. Lower jaw panorama, panoramic x-ray, dental structure...": "مثال: بانوراما الفك السفلي، أشعة بانورامية، بنية الأسنان...",
            "API Auth Token": "رمز تفويض API Auth Token",
            "Stream ID": "معرف القناة Stream ID",
            "Your CommPeak API Auth Token (Bearer token)": "رمز تفويض CommPeak API الخاص بك (Bearer token)",
            "Your CommPeak Stream ID": "معرف قناة CommPeak الخاص بك (Stream ID)",
            "Enter your TextPeak API Auth Token and Stream ID.": "أدخل رمز تفويض API ومعرف القناة (Stream ID) الخاص بـ TextPeak.",
            "Register at commpeak.com — supports Syriatel & MTN Syria.": "سجل في موقع commpeak.com - يدعم سيريتيل و MTN سوريا.",
            ...
            "From Email Address": "عنوان البريد الإلكتروني للمرسل",
            "Test Chat ID (your Telegram chat_id)": "معرف دردشة تيليغرام للتجربة",
        ...
        function translateDOM(node) {
            if (node.nodeType === Node.TEXT_NODE) {
                ...
                // Skip translation if parent element is marked notranslate or translate="no"
                if (node.parentElement && (node.parentElement.closest('[translate="no"]') || node.parentElement.closest('.notranslate'))) {
                    return;
                }

                // If text node already contains Arabic characters, skip JS replacement to prevent mangled/double translation
                if (/[\u0600-\u06FF]/.test(rawText)) {
                    return;
                }
                ...
                // Skip translating the script blocks, style blocks, BDI tags, sitename and notranslate elements
                if (node.tagName === 'SCRIPT' || node.tagName === 'STYLE' || node.tagName === 'BDI' || node.getAttribute('translate') === 'no' || (node.classList && (node.classList.contains('sitename') || node.classList.contains('notranslate')))) return;
